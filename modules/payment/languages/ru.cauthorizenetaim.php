...
define('CAUTHORIZENETAIM_DSCR', 
	'Обработка кредитных карт через платежную систему Authorize.Net по методу Advanced Integration Method (AIM).<br>Информация о кредитной карте вводится покупателем на вашем сайте и затем передается на сервер Authorize.Net.<br>Для работы модуля необходимо, чтобы на сервере был установлен модуль PHP CURL и SSL-сертификат для домена, на котором работает ваш магазин.');
