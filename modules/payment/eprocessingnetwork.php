...
//	define( 'EPN_ENDPOINT_URL_TEST', 'https://certification.authorize.net/gateway/transact.dll' );
	...
	function EPN_sendData( $variables ) //send data to Authorize.Net server (CURL)
	{
		if ( !($ch = curl_init()) ){
			...
			CEProcessingNetwork::_writeLogMessage(MODULE_LOG_CURL, 'Local error: '.ERR_CURLINIT);
			return ERR_CURLINIT;
		}

		if ( curl_errno($ch) != 0 ){
			
			CEProcessingNetwork::_writeLogMessage(MODULE_LOG_CURL, 'Curl error: '.curl_errno($ch));
			return ERR_CURLINIT;
		}
		$vars = "";
	...
	function after_processing_php($orderID)
	{
		if ($this->_getSettingValue('CONF_PAYMENTMODULE_EPROCNETWORK_SAVE_CC_INFORMATION')) // save credit card data
		{
			...
			if ($orderID)
			{
				...
				db_query("update ".ORDERS_TABLE." set cc_number = '".cryptCCNumberCrypt($_POST["mEPN_cc_number"],null)."', cc_holdername = '".cryptCCHoldernameCrypt($_POST["mEPN_cc_holder"],null)."', cc_expires = '".cryptCCExpiresCrypt($expires,null)."', cc_cvv = '".cryptCCNumberCrypt($cvv,null)."' where orderID=$orderID") or die (db_error());
			}
		}
		return "";
	}
