...
function quickOrderContactInfoVerify()
{
	...
	//if (     !eregi("^[_\.0-9a-zA-Z-]+@([0-9a-zA-Z][0-9a-zA-Z-]+\.)+[a-zA-Z]{2,6}$/i", $Email) )	
...
function quickOrderReceiverAddressVerify()
{
	...
	$receiver_middle_name	= $_POST["receiver_middle_name"];
	...
	if ( isset($_POST["zoneID"]) )
		...
		$zoneID = 0; 
	$error = regVerifyAddress( $receiver_first_name, $receiver_middle_name, $receiver_last_name, $countryID, $zoneID, $state, 
									$zip, $city, $address );
	return $error;
}
...
function quickOrderBillingAddressVerify()
{
	...
	$payer_middle_name		= $_POST["payer_middle_name"];
	...
	if ( isset($_POST["billingZoneID"]) )
		...
		$billingZoneID = 0;
	$error = regVerifyAddress( $payer_first_name, $payer_middle_name, $payer_last_name, $billingCountryID, 
							$billingZoneID, $billingState, $billingZip, 
...
function quickOrderSetReceiverAddress()	
{
	...
	$_SESSION["receiver_middle_name"] = $_POST["receiver_middle_name"];
	$_SESSION["receiver_last_name"]  = $_POST["receiver_middle_name"]." ".$_POST["receiver_last_name"];
	...
	/*echo 'quickOrderSetReceiverAddress $_SESSION<pre>';
		print_r($_SESSION);
	echo '<pre>';
	exit();*/
}
...
function quickOrderSetBillingAddress()
{
	...
	if ( !isset($_POST["billing_address_check"]) )
	{
		...
		$_SESSION["billing_middle_name"]  = $_POST["payer_middle_name"];
		$_SESSION["billing_last_name"]	 = $_POST["payer_middle_name"]." ".$_POST["payer_last_name"];
	...
	else
	{
		...
		$_SESSION["billing_middle_name"]  = $_POST["receiver_middle_name"];
		$_SESSION["billing_last_name"]   = $_POST["receiver_middle_name"]." ".$_POST["receiver_last_name"];
...
function quickOrderGetReceiverAddressStr()
{
	...
	$strAddress = 
				xHtmlSpecialChars($_SESSION["receiver_first_name"] )." ".
				xHtmlSpecialChars( $_SESSION["receiver_last_name"] );
	...
	if (strlen($_SESSION["receiver_zip"])>0)
...
function quickOrderGetBillingAddressStr()
{
	...
	$strAddress = 
				xHtmlSpecialChars( $_SESSION["billing_first_name"] )." ".
				xHtmlSpecialChars( $_SESSION["billing_last_name"] );
	...
	if (strlen($zone)>0)
		$strAddress .= " ".xHtmlSpecialChars($zone);
	if (strlen($_SESSION["billing_zip"])>0)
