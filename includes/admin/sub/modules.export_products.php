	...
	switch ($ACTION){
		...
		case 'UPDATE_CATEGORY_STATE':
			...
			if(!$_SESSION['checkedCategories'][$_CategoryID]){
				...
				$Count = 0;
				$callBackParam = array();
				$callBackParam["categoryID"] = $_CategoryID;
				$callBackParam["searchInSubcategories"] = false;
				$Products = prdSearchProductByTemplate($callBackParam,$Count);
			...
			if(!$_SESSION['isExpanded'][$_CategoryID]){
				...
				for ($i=0; $i<$c;$i++){
					...
					if(!$_SESSION['checkedCategories'][$_SubC[$i]]){
						...
						$Count = 0;
						$callBackParam = array();
						$callBackParam["categoryID"] = $_SubC[$i];
						$callBackParam["searchInSubcategories"] = false;
						$Products = prdSearchProductByTemplate($callBackParam,$Count);
...
if(isset($_POST['showProducts'])){
	...
	$Count = 0;
	$callBackParam = array();
	$callBackParam["categoryID"] = intval($_POST['showProducts']);
	$callBackParam["searchInSubcategories"] = false;
	$Products = prdSearchProductByTemplate($callBackParam,$Count);
...
/**
 * Count selected products and total products in category
 */
$OldErrorReporting = error_reporting(0);
foreach ($ProductCategories as $Index => $ProductCategory){

	$selectedProductsNumber = @count($_SESSION['selectedProducts'][$ProductCategory['categoryID']]);
	if($ProductCategory['ExpandedCategory'] || !$ProductCategory['ExistSubCategories']){
		
		$ProductCategories[$Index]['_total_products_num'] = $ProductCategory['products_count_category'];
		
		if($selectedProductsNumber){
			
			$ProductCategories[$Index]['_selected_products_num'] = $selectedProductsNumber;
		}elseif ( $_SESSION['checkedCategories'][$ProductCategory['categoryID']]){
			
			$ProductCategories[$Index]['_selected_products_num'] = $ProductCategory['products_count_category'];
		}else{
			
			$ProductCategories[$Index]['_selected_products_num'] = 0;
		}
		
	}else{
		
		$ProductCategories[$Index]['_total_products_num'] = $ProductCategory['products_count'];
		
		if ($_SESSION['selectedProductsIncSub'][$ProductCategory['categoryID']]) {
			
			if($_SESSION['selectedProductsIncSub'][$ProductCategory['categoryID']]>$ProductCategories[$Index]['_total_products_num']){
				
				$_SESSION['selectedProductsIncSub'][$ProductCategory['categoryID']] = $ProductCategories[$Index]['_total_products_num'];
			}
			$ProductCategories[$Index]['_selected_products_num'] = $_SESSION['selectedProductsIncSub'][$ProductCategory['categoryID']];
		}else {
