...
if ( !strcmp($sub,"shipping") )
{
	...
	if ( isset($_GET["setting_up"]) )
	{
		if (isset($_POST) && count($_POST)>0)
		{
			...
			{
				Redirect( "admin.php?dpt=modules&sub=shipping&setting_up=".$_GET["setting_up"]."&safemode=yes" );
			}
		}
	...
	else
	{
		...
		if ( isset($_GET["install"]) )
		{
			if (CONF_BACKEND_SAFEMODE) //this action is forbidden when SAFE MODE is ON
			{
				Redirect( "admin.php?dpt=modules&sub=shipping&safemode=yes" );
			}

			$shipping_modules[ (int)$_GET["install"] ]->install();
			Redirect( "admin.php?dpt=modules&sub=shipping" );
		}
