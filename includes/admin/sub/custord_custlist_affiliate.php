	...
	if (CONF_BACKEND_SAFEMODE && isset($_POST['fACTION'])) //this action is forbidden when SAFE MODE is ON
	{
		...
		switch ($sub_page) {
			...
			default:
				 ...
				 */
				if (isset($_POST['from']))$_GET['from'] = xStripSlashesGPC($_POST['from']);
				if (isset($_POST['till']))$_GET['till'] = xStripSlashesGPC($_POST['till']);
				if (!isset($_GET['from']))$_GET['from'] = '';
				else $_GET['from'] = xStripSlashesGPC(rawurldecode($_GET['from']));
				if (!isset($_GET['till']))$_GET['till']='';
				else $_GET['till'] = xStripSlashesGPC(rawurldecode($_GET['till']));
				...
				if(isset($show_tables)){
					...
					$Payments = affp_getPayments(
						...
						TransformStringToDataBase($_GET['OrderField'].' '.$_GET['OrderDiv']));
					...
					$Commissions = affp_getCommissions(
						...
						TransformStringToDataBase($_GET['OrderFieldC'].' '.$_GET['OrderDivC']));
