...
# //$sfWatermark path to 24b - png  
 function SetWatermark($rImg, $sType, $sfWatermark = '/virt/homes/prokat/htdocs/images/watermark.png'){  
     $iDelta = 5;
    // echo $rImg;  
     $xImg = imagesx($rImg);  
     $yImg = imagesy($rImg);  
...
//echo '<br>tyt';
//exit();
			if($newfile === FIRST_NAME)
			{
				//echo '<br>tyt';
				if(@preg_match("/\..*+$/",@basename($this->imgFile),$matches))
				{
					$newfile=@substr_replace($this->imgFile,"",-@strlen($matches[0]),0);
					$_SESSION['z_photo_100'] = $newfile;
					//echo "<br>$_SESSION[z_photo_100] = ".$_SESSION['z_photo_100'];
				}

			}
			elseif ($newfile === SECOND_NAME)
			{
				if(@preg_match("/\..*+$/",@basename($this->imgFile),$matches))
				{
					$newfile=@substr_replace($this->imgFile,"_".$_SESSION['timestamp']."_300",-@strlen($matches[0]),0);
					$_SESSION['z_photo_300'] = $newfile;
